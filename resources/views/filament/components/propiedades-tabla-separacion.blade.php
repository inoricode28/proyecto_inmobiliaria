        ...
        <div class="flex items-center gap-4">
            <div class="flex-1">
                ...
                <select id="property-selector" class="w-full border border-gray-300 rounded-md px-3 py-2 bg-white">
                    ...
                    @endphp
                    <option value="{{ $inmuebleId }}" data-proyecto="{{ $departamento->proyecto->nombre ?? 'N/A' }}"
        ...
        <table class="w-full border-collapse border border-gray-300 table-fixed">
            <thead>
                <tr class="bg-gray-100">
                    ...
                    <th class="border border-gray-300 px-4 py-2 text-right bg-gray-50" style="width: 120px;">Precio
                        Lista</th>
                    ...
                    <th class="border border-gray-300 px-4 py-2 text-right bg-gray-50" style="width: 140px;">Saldo a
                        Financiar</th>
                </tr>
            </thead>
            <tbody id="properties-tbody">
                ...
                <tr id="default-row" class="text-gray-500 italic">
                    ...
                    <td class="border border-gray-300 px-4 py-2 text-right bg-gray-50" style="width: 120px;">S/ 0.00
                    </td>
                    ...
                    <td class="border border-gray-300 px-4 py-2 text-right" style="width: 120px;">S/ 0.00</td>
                    <td class="border border-gray-300 px-4 py-2 text-right" style="width: 120px;">S/ 0.00</td>
                    <td class="border border-gray-300 px-4 py-2 text-right bg-gray-50" style="width: 140px;">S/ 0.00
                    </td>
                </tr>
            </tbody>
            <tfoot>
...
function restoreProperties() {
    ...
    if (window.loadedProperties && window.loadedProperties.length > 0 &&
        ...
        console.log('🚨 ¡Propiedades realmente desaparecieron! Restaurando', window.loadedProperties.length,
            'propiedades');
...
if (window.morphdom) {
    ...
    const checkMorphdom = setInterval(() => {
        if (window.morphdom) {
            ...
            window.morphdom = function(fromNode, toNode, options = {}) {
                ...
                options.onBeforeElUpdated = function(fromEl, toEl) {
                    ...
                    if (fromEl.hasAttribute && (
                        ...
                        console.log('🛡️ MORPHDOM INTERCEPTADO (tardío): Protegiendo fila dinámica',
                            fromEl);
                        return false;
                    }
                    ...
                    if (fromEl.id === 'properties-tbody') {
                        console.log(
                            '🛡️ MORPHDOM INTERCEPTADO (tardío): Protegiendo tbody de propiedades');
                        return false;
                    }
            ...
            console.log(
                '🛡️ Interceptor de morphdom mejorado instalado tardíamente con restauración automática');
    ...
    window.Livewire.on('component.updated', (component) => {
        ...
        setTimeout(() => {
            ...
            if (tbody) {
                ...
                console.log('🔍 Filas dinámicas encontradas después de actualización:', dynamicRows
                    .length);
            ...
            mutations.forEach(function(mutation) {
                if (mutation.type === 'childList' && mutation.removedNodes.length > 0) {
                    mutation.removedNodes.forEach(function(node) {
                        if (node.nodeType === 1 && node.tagName === 'TR' && node
                            .dataset && node.dataset.propertyId && node.dataset
                            .separacionId) {
                            console.warn('⚠️ Fila con separación removida:', node
                                .dataset.propertyId);
                        }
                    });
                }
    ...
    setTimeout(() => {
        ...
        if (currentRows === 0) {
            ...
            const retryInterval = setInterval(() => {
                const tbody = document.getElementById('properties-tbody');
                const currentRows = tbody ? tbody.querySelectorAll('tr:not(#default-row)')
                    .length : 0;

                if (currentRows === 0 && retryCount < maxRetries) {
                    retryCount++;
                    console.log(
                        `🔄 Reintento ${retryCount}/${maxRetries} - Cargando propiedades...`
                        );
                    loadPropertiesFromAPI();
                    ...
                    if (currentRows > 0) {
                        ...
                        console.log(
                            '⚠️ No se pudieron cargar propiedades después de varios intentos'
                            );
                    }
                }
            }, 2000);
            ...
            if (propiedadesConSeparacion.length > 0) {
                ...
                setTimeout(() => {
                    ...
                    if (visibleRows === 0 && propiedadesConSeparacion.length > 0) {
                        console.log(
                            '⚠️ Las propiedades se cargaron pero no son visibles, forzando restauración...'
                            );
                    ...
                    setTimeout(() => {
                        ...
                        if (addedProperties.length > 0 && rows2.length === 0) {
                            console.error(
                                '🚨 PROBLEMA CONFIRMADO: Las propiedades han desaparecido!');
                            ...
                            if (!tbody2) {
                                ...
                                console.log('📋 🔍 Verificando estado de propiedades:', {
                                    loadedProperties: window.loadedProperties?.length || 0,
                                    currentDynamicRows: tbody2.querySelectorAll(
                                        '[data-dynamic-row], [data-property-id], [data-index]'
                                        ).length,
...
function populatePropertySelector(propiedades) {
    ...
    propiedades.forEach(propiedad => {
        ...
        option.textContent =
            `${propiedad.proyecto} - ${propiedad.numero} (${propiedad.tipo} - ${propiedad.dormitorios} dorm.)${separacionText}`;
...
function addSelectedProperty() {
    ...
    console.log('🔄 Agregando propiedad seleccionada:', propertyData);

    // AGREGAR AL ARRAY GLOBAL DE INMUEBLES SELECCIONADOS
    if (!window.inmueblesSeleccionados) {
        window.inmueblesSeleccionados = [];
    }

    // Verificar si ya está en el array global
    if (window.inmueblesSeleccionados.includes(propertyData.departamento_id)) {
        alert('Este inmueble ya ha sido agregado.');
        return;
    }
    ...
    const successDepartamento = updateFilamentField('departamento_id', propertyData.departamento_id);

    if (!successDepartamento) {
        console.warn('⚠️ No se pudo actualizar el campo departamento_id en el formulario');
    }

    // También actualizar otros campos relacionados con el inmueble seleccionado
    updateFilamentField('proyecto_nombre', propertyData.proyecto);
    updateFilamentField('departamento_nombre', propertyData.numero);
    updateFilamentField('precio_lista', propertyData.precio);
    updateFilamentField('descuento', propertyData.descuento);

    // Calcular precio de venta
    const precioVenta = propertyData.precio - (propertyData.precio * propertyData.descuento / 100);
    updateFilamentField('precio_venta', precioVenta);

    updateFilamentField('monto_separacion', propertyData.separacion);
    updateFilamentField('cuota_inicial', propertyData.cuotaInicial);

    // Calcular saldo a financiar
    const saldoFinanciar = precioVenta - propertyData.separacion - propertyData.cuotaInicial;
    updateFilamentField('saldo_financiar', saldoFinanciar);

    console.log('✅ Campos del formulario actualizados para el inmueble seleccionado:', {
        departamento_id: propertyData.departamento_id,
        proyecto: propertyData.proyecto,
        numero: propertyData.numero,
        inmuebles_seleccionados: window.inmueblesSeleccionados
    });

    // Agregar a la tabla
    addPropertyToTable(propertyData);

    // Agregar a la lista de propiedades agregadas (para control del frontend)
    ...
    console.log(`📊 Resumen: ${window.inmueblesSeleccionados.length} inmueble(s) seleccionado(s):`, window
...
function addPropertyToTable(propertyData) {
    ...
    console.log('🔍 Verificando que la fila esté en el DOM:', document.querySelector(`[data-index="${index}"]`) ? 'SÍ' :
        'NO');
...
function updateRowCalculations(index) {
    ...
    row.querySelector('.precio-venta').textContent = 'S/ ' + precioVenta.toLocaleString('es-PE', {
        minimumFractionDigits: 2
    });
    row.querySelector('.precio-venta').dataset.value = precioVenta;
    row.querySelector('.saldo-financiar').textContent = 'S/ ' + saldoFinanciar.toLocaleString('es-PE', {
        minimumFractionDigits: 2
    });
...
function updateCalculations() {
    ...
    rows.forEach(row => {
        ...
        const separacion = separacionSpan ? parseFloat(separacionSpan.textContent.replace(/[^\d.-]/g, '')) ||
            0 : 0;
        const cuotaInicial = cuotaInicialSpan ? parseFloat(cuotaInicialSpan.textContent.replace(/[^\d.-]/g,
            '')) || 0 : 0;
    ...
    Object.entries(totalElements).forEach(([id, value]) => {
        ...
        if (element) {
            element.textContent = 'S/ ' + value.toLocaleString('es-PE', {
                minimumFractionDigits: 2
            });
        }
    });
...
function getExistingSeparaciones(multipleData) {
    ...
    for (const property of multipleData.properties) {
        ...
        if (row && row.dataset.separacionId) {
            console.log(
                `✅ Separación existente para departamento ${property.departamento_id}: ${row.dataset.separacionId}`);
    ...
    try {
        ...
        try {
            ...
            if (separacionesStatus.allExist) {
                ...
                console.log(
                    `⚠️ Faltan ${separacionesStatus.needToCreate.length} separaciones de ${multipleData.properties.length} total`
                    );
                console.log(
                    '📋 Para acceder al cronograma de cuota inicial, primero debe crear las separaciones desde los otros modales'
                    );
...
async function openSFModalWithSeparaciones() {
    try {
        ...
        if (window.multiplePropertiesData && window.multiplePropertiesData.properties) {
            console.log('✅ Datos múltiples confirmados - propiedades:', window.multiplePropertiesData.properties
                .length);
        } else {
    ...
    try {
        ...
        for (const property of multipleData.properties) {
            const row = document.querySelector(
                `#properties-tbody tr[data-departamento-id="${property.departamento_id}"]`);
            if (row && row.dataset.separacionId) {
                console.log(
                    `✅ Separación ya existe para departamento ${property.departamento_id}: ${row.dataset.separacionId}`
                    );
                existingSeparaciones.push({
                    departamento_id: property.departamento_id,
                    id: row.dataset.separacionId
                ...
                console.log(
                    `⚠️ Separación no existe para departamento ${property.departamento_id}, necesita crearse`);
                needToCreate.push(property);
            }
        }
        ...
        if (needToCreate.length === 0) {
            ...
            return {
                success: true,
                message: 'Separaciones ya existentes',
                data: {
                    separaciones: existingSeparaciones
                }
            };
        }

        console.log(
            `🔄 Necesario crear ${needToCreate.length} separaciones de ${multipleData.properties.length} total`);
...
function getMultiplePropertiesData() {
    ...
    // VERIFICAR INMUEBLES SELECCIONADOS PRIMERO
    console.log('🎯 === VERIFICANDO INMUEBLES SELECCIONADOS ===');
    console.log('📊 window.inmueblesSeleccionados:', window.inmueblesSeleccionados);
    ...
    // SOLO PROCESAR FILAS DE INMUEBLES SELECCIONADOS
    const properties = [];
    let totalPrecioLista = 0;
    let totalDescuento = 0;
    let totalPrecioVenta = 0;
    ...
    console.log(`🎯 Procesando ${window.inmueblesSeleccionados.length} inmuebles seleccionados`);

    window.inmueblesSeleccionados.forEach((departamentoId, index) => {
        console.log(`🏠 === PROCESANDO INMUEBLE SELECCIONADO ${index + 1}: ID ${departamentoId} ===`);

        // Buscar la fila específica de este departamento
        const row = document.querySelector(`#properties-tbody tr[data-departamento-id="${departamentoId}"]`) ||
            document.querySelector(`#properties-tbody tr[data-property-id="${departamentoId}"]`) ||
            document.querySelector(`#properties-tbody tr[data-index="${departamentoId}"]`);

        if (!row) {
            console.error(`❌ NO SE ENCONTRÓ FILA PARA DEPARTAMENTO ID: ${departamentoId}`);
            return;
        }

        console.log(`✅ Fila encontrada para departamento ${departamentoId}`);
        ...
        const cuotaInicial = cuotaInicialSpan ? parseFloat(cuotaInicialSpan.textContent.replace(/[^\d.-]/g,
            '')) || 0 : 0;
        const saldoFinanciar = saldoFinanciarSpan ? parseFloat(saldoFinanciarSpan.textContent.replace(
            /[^\d.-]/g, '')) || 0 : 0;

        // Obtener información del inmueble
        const inmuebleInfo = row.querySelector('td:nth-child(3)');
        const inmuebleNumero = inmuebleInfo ? (inmuebleInfo.querySelector('.font-medium')?.textContent || 'N/A')
            .trim().replace(/\s+/g, ' ') : 'N/A';
        ...
        console.log(`   ✅ DATOS DEL INMUEBLE ${departamentoId}:`);
...
// Función para actualizar campos del formulario Filament
function updateFilamentField(fieldName, value) {
    try {
        console.log(`🔄 Actualizando campo ${fieldName} con valor:`, value);

        // Buscar el campo por múltiples selectores
        const selectors = [
        ...
        if (field) {
            ...
            // Para campos select, también disparar el evento input
            ...
            console.log(`✅ Campo ${fieldName} actualizado correctamente a:`, value);
            return true;
        } else {
            console.warn(`⚠️ Campo ${fieldName} no encontrado en el formulario. Selectores probados:`, selectors);

            // Intentar encontrar campos ocultos
            const hiddenFields = document.querySelectorAll('input[type="hidden"]');
            let hiddenFieldFound = false;
