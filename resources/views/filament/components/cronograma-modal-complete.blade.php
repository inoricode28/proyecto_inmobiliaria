...
<div>
    <div id="cronograma-modal" class="fixed inset-0 z-50 overflow-y-auto hidden" aria-labelledby="modal-title"
            ...
            <div
                ...
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    ...
                    <div id="cronograma-content">
                        ...
                        <div class="space-y-4 mb-6">
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                ...
                                <div>
                                    ...
                                    <input type="number" id="montoTotal"
                                        class="w-full p-2 border border-gray-300 rounded-lg bg-gray-100" step="0.01"
                                        readonly>
                                </div>
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">Número de
                                        Cuotas:</label>
                                    <input type="number" id="numeroCuotas"
                                        class="w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                            ...
                            <div class="flex justify-center">
                                <button type="button" id="generarCuotas"
                    ...
                    <div class="flex justify-end space-x-3 mt-6 pt-4 border-t">
                        ...
                        <button type="button" id="guardarCronograma"
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        window.showCronogramaModal = function() {
            ...
            console.log('🔍 Estado actual de window.multiplePropertiesData:', window
            .multiplePropertiesData);
            ...
            if (modal) {
                ...
                setTimeout(() => {
                    ...
                    loadProformaData
                (); // Luego cargar datos de proforma (sin generar cuota por defecto si ya hay cuotas)
                    setDefaultDate();
                },
            ...
            window.openCronogramaModal = async function() {
                ...
                if (!window.multiplePropertiesData && typeof getMultiplePropertiesData === 'function') {
                    ...
                    try {
                        ...
                        if (multipleData && multipleData.properties && multipleData.properties.length >
                            0) {
                            ...
                            console.log('✅ window.multiplePropertiesData establecido:', window
                                .multiplePropertiesData);
                        }
                    } catch (error) {
        ...
        function getCurrentProformaId() {
            ...
            if (!proformaId) {
                ...
                if (filamentForm) {
                    const proformaField = filamentForm.querySelector(
                        'select[name="proforma_id"], input[name="proforma_id"]');
                    ...
                    if (tiposData.success) {
                        ...
                        cuotas.forEach((cuota, index) => {
                            ...
                            tiposCuota.forEach(tipo => {
                                const selected = (tipo.id == cuota.tipo_cuota_id) ?
                                    'selected' : '';
                                tiposOptions +=
                            ...
                            const fechaFormateada = new Date(cuota.fecha_pago).toISOString().split(
                                'T')[0];
                        ...
                        `;
                            ...
                            row.querySelector('.eliminar-cuota').addEventListener('click',
                            function() {
                                if (confirm('¿Está seguro de eliminar esta cuota?')) {
                                    ...
                                    if (cuotasTableBody.children.length === 0) {
                                        document.getElementById('cuotasSection').classList
                                            .add('hidden');
                                    }
                                }
                            });
        ...
        function loadProformaData() {
            ...
            setTimeout(() => {
                console.log('🔍 Verificando window.multiplePropertiesData (después del delay):', window
                    .multiplePropertiesData);

                // Verificar si hay datos de múltiples propiedades
                if (window.multiplePropertiesData && window.multiplePropertiesData.totals) {
                    console.log('📦 Usando datos de múltiples propiedades:', window
                        .multiplePropertiesData);
                    updateModalElementsWithMultipleData(window.multiplePropertiesData);
                    return;
                } else {
                    console.log(
                        '⚠️ No se encontraron datos de múltiples propiedades, usando lógica individual'
                        );
        ...
        function proceedWithIndividualLogic() {
            ...
            if (proformaId && proformaId !== '' && proformaId !== '0') {
                ...
                for (let element of allElements) {
                    if (element.value) {
                        console.log(
                            `- ${element.tagName} [${element.name || element.id || 'sin-nombre'}]: ${element.value}`
                            );
                    }
                }
            }
            ...
            fetch('/api/cronograma/tipos-cuota')
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        tiposCuota.forEach(tipo => {
                            if (tipo.nombre && tipo.nombre.toLowerCase().includes(
                                'cuota inicial')) {
                        ...
                        let tiposOptions = '';
                        tiposCuota.forEach(tipo => {
                            const selected = (cuotaInicialId && tipo.id === cuotaInicialId) ?
                                'selected' : '';
                            tiposOptions +=
                                `<option value="${tipo.id}" ${selected}>${tipo.nombre}</option>`;
                        });
                    ...
                    `;
                        ...
                        row.querySelector('.eliminar-cuota').addEventListener('click', function() {
                            if (confirm('¿Está seguro de eliminar esta cuota?')) {
                                ...
                                // Si no quedan cuotas, ocultar la sección
                                if (cuotasTableBody.children.length === 0) {
                                    document.getElementById('cuotasSection').classList.add(
                                    'hidden');
                                }
                            }
                        });
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        for (let i = 0; i < numeroCuotas; i++) {
                            ...
                            tiposCuota.forEach(tipo => {
                                const selected = (cuotaInicialId && tipo.id ===
                                    cuotaInicialId) ? 'selected' : '';
                                tiposOptions +=
                                    `<option value="${tipo.id}" ${selected}>${tipo.nombre}</option>`;
                            });
                        ...
                        document.querySelectorAll('.eliminar-cuota').forEach(btn => {
                            btn.addEventListener('click', function() {
                                if (confirm(
                                    ...
                                    // Si no quedan cuotas, ocultar la sección
                                    if (cuotasTableBody.children.length === 0) {
                                        document.getElementById('cuotasSection')
        ...
        function guardarCronogramaCompleto() {
            ...
            // Si no se encuentra, intentar obtener de la URL (directo) antes de usar el parámetro "from"
            if (!proformaId) {
                const urlParams = new URLSearchParams(window.location.search);
                const urlProformaId = urlParams.get('proforma_id') || urlParams.get('id');
                if (urlProformaId) {
                    proformaId = urlProformaId;
                    // console.log('✅ Proforma ID obtenido directamente de la URL:', proformaId);
                }
            }

            // Como último recurso, intentar obtener del parámetro "from" si indica separación definitiva
            if (!proformaId) {
                const urlParams = new URLSearchParams(window.location.search);
                ...
                if (fromParam && fromParam.includes('separacion_definitiva:')) {
                    ...
                    if (parts.length >= 2) {
                        proformaId = parts[1];
                        // console.log('✅ Proforma ID obtenido de parámetro "from":', proformaId);
                    }
            ...
            // Normalizar proformaId a número entero para cumplir validaciones
            if (proformaId) {
                proformaId = parseInt(proformaId, 10);
            ...
            if (!proformaId) {
                console.error('❌ No se pudo encontrar el ID de la proforma. Selectores probados:',
                    proformaSelectors);
                // console.log('🔍 Elementos del formulario disponibles:');
                const allInputs = document.querySelectorAll('input, select');
                allInputs.forEach(input => {
                    if (input.name && (input.name.includes('proforma') || input.name.includes(
                            'cliente'))) {
            ...
            rows.forEach(row => {
                ...
                const tipoCuotaId = tipoSelect.value ? parseInt(tipoSelect.value, 10) : null;
            ...
            fetch('/cronograma/guardar', {
                ...
                .then(data => {
                    ...
                    if (data.success) {
                        alert('Cronograma guardado exitosamente' + (data.data.separacion_creada ?
                            ' (Separación creada automáticamente)' : ''));
                        // Cerrar el modal
                ...
                .then(tiposData => {
                    if (tiposData.success && tiposData.data) {
                        ...
                        cuotas.forEach((cuota, index) => {
                            ...
                            fechaCell.innerHTML =
                                `<input type="date" class="form-control" value="${cuota.fecha_pago || ''}" name="cuotas[${currentRowIndex}][fecha_pago]">`;
                            row.appendChild(fechaCell);

                            // Monto
                            const montoCell = document.createElement('td');
                            montoCell.innerHTML =
                                `<input type="number" class="form-control" value="${cuota.monto || 0}" step="0.01" name="cuotas[${currentRowIndex}][monto]">`;
                            ...
                            tipoCell.innerHTML =
                                `<select class="form-control" name="cuotas[${currentRowIndex}][tipo_cuota_id]">${tipoOptions}</select>`;
                            row.appendChild(tipoCell);

                            // Acciones
                            const accionesCell = document.createElement('td');
                            accionesCell.innerHTML =
                                `<button type="button" class="btn btn-danger btn-sm" onclick="eliminarCuota(this)"><i class="fas fa-trash"></i></button>`;
        ...
        function updateModalElementsWithMultipleData(multipleData) {
            ...
            const cuotaInicialElement = document.getElementById('cuota-inicial');
            if (cuotaInicialElement) {
                cuotaInicialElement.textContent = 'S/ ' + multipleData.totals.cuota_inicial.toLocaleString(
                    'es-PE', {
