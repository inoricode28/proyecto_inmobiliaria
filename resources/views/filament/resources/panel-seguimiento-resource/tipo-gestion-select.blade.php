@php
    ...
    // Obtener conteos iniciales (sin filtros)
    $conteosIniciales = $this->getConteosParaVista();

    // Obtener conteos filtrados si hay filtros activos
    $filtrosActivos = array_filter([
        'proyecto' => $this->form->getState()['proyecto'] ?? null,
        'usuario_id' => $this->form->getState()['usuario_id'] ?? null,
        'comoSeEntero' => $this->form->getState()['comoSeEntero'] ?? null,
        'fechaInicio' => $this->form->getState()['fechaInicio'] ?? null,
        'fechaFin' => $this->form->getState()['fechaFin'] ?? null,
    ], fn($value) => !empty($value) && $value != 0);

    $conteos = empty($filtrosActivos)
        ? $conteosIniciales
        : $this->getConteosParaVista($this->form->getState());
...
<div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 lg:grid-cols-6 gap-3 w-full">
    ...
    <button type="button" wire:click="$set('{{ $getStatePath() }}', {{ $tipo->id }})" class="flex flex-col items-center justify-center p-3 rounded-lg transition-all h-28
                {{ $isSelected
        ...
        <span class="mt-1 text-lg font-bold">{{ $conteo }}</span>
    </button>
    @endforeach
</div>
